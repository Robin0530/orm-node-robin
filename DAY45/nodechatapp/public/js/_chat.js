...
$("#contacts-tab").click(function () {
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      if (result.code == 200) {
				// 최초 ul태그내 li태그 모두 삭제처리 / contacts-list의 html 초기화 // 안하면 화면에 누적되서 쌓임
				$(".contacts-list").html("");
...
$("#btnSend").click(function () {
  ...
  // 임시로 현재 사용자 닉네임사용-추후 토큰에서 정보 추출하기
	// 현재 접속한 채널 고유번호 조회
...
socket.on("receiveTest", function (nickName, msg, sendDate) {
  // 현재 로그인한 사용자의 이름값과 nickName 값이 같으면 어떤값이 "chat-right"에 들어가고 아니면 "chat-left"에 들어간다.
  // 오른쪽 템플릿을 쓸지, 왼쪽을 쓸지..
...
// 채팅방 메시지 수신 처리하기
socket.on("reciveChannelMsg", function (nickName, profile, msg, sendDate) {
  // 현재 로그인한 사용자의 이름값과 nickName 값이 같으면 어떤값이 "chat-right"에 들어가고 아니면 "chat-left"에 들어간다.
  // 오른쪽 템플릿을 쓸지, 왼쪽을 쓸지..
	var msgTag = currentUser.name == nickName ? 
	`<li class='chat-right'>
