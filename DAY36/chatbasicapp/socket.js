...
module.exports =(server)=>{
    ...
    */
    ...
    io.on("connection", async(socket)=>{
        ...
        // 서버측/클라이언트측 이벤트 수신기명과 전달 데이터수/포맷은 개발자가 정의한다(메시지수)
        ...
        socket.on("entry", async(channelId, nickName)=>{
            ...
            // socket.join()동일 채널 id가 없으면 해당 채팅방을 만들고 있으면 해당채널로 접속한다.
            socket.join(channelId);

            // 채널 입장사실을 사용자들에게 알려주기 3가지 방법

            // CASE1: 현재 접속한 사용자들에게만 메시지를 보내기
            // socket.emit("클라이언트 이벤트 수신기명", 전달데이터)
            socket.emit("entryok", `${nickName} 대화명으로 입장했습니다.`);


            // CASE2: 현재 접속한 채팅방에 나를(현재접속중인) 제외한 나머지 사용자들에게만 메시지를 보내고 싶을때
            // 현재 접속자를 제외한 같은 채팅방내 모든 사용자에게 메시지 발송
            // socket.to('채널아이디').emit('클라이언트 이벤트수신기명', 전송데이터): 해당채널에 접속중인 나를 제외한 
